...
async function checkAndCreateTables() {
  ...
  try {
    ...
    try {
      ...
      if (error && error.code === '42P01') { // SQLSTATE 42P01: tabela não existe
        ...
        console.log(`Tabela grupos_estudo existe com ${count} registros`);
      }
    } catch (e) {
      console.error('Exceção ao verificar tabela grupos_estudo:', e);
      await createGruposEstudoTable();
    }
    
    // Tabela codigos_grupos_estudo
    try {
      const { count, error } = await supabase
      ...
      if (error && error.code === '42P01') { // SQLSTATE 42P01: tabela não existe
        ...
        await createCodigosGruposEstudoTable();
      } else if (error) {
        console.error('Erro ao verificar tabela codigos_grupos_estudo:', error);
      } else {
        console.log(`Tabela codigos_grupos_estudo existe com ${count} registros`);
      }
    } catch (e) {
      console.error('Exceção ao verificar tabela codigos_grupos_estudo:', e);
      await createCodigosGruposEstudoTable();
    }
      ...
      try {
        await supabase.query(`
          ...
          CREATE POLICY "Usuários podem excluir grupos"
            ...
            USING (true);
        `);
        console.log('Políticas de segurança criadas para grupos_estudo');
      } catch (policyError) {
        console.error('Erro ao criar políticas de segurança:', policyError);
      }
    }
  } catch (e) {
    console.error('Exceção ao criar tabela grupos_estudo:', e);
  }
}

async function createCodigosGruposEstudoTable() {
  try {
    const { error } = await supabase.query(`
    ...
    if (error) {
      ...
      try {
        ...
        `);
        console.log('Função e trigger criados para codigos_grupos_estudo');
      } catch (triggerError) {
        console.error('Erro ao criar função/trigger:', triggerError);
      }
      ...
      try {
        await supabase.query(`
          ...
          CREATE POLICY "Todos podem atualizar códigos"
            ...
            USING (true);
        `);
        console.log('Políticas de segurança criadas para codigos_grupos_estudo');
      } catch (policyError) {
        console.error('Erro ao criar políticas de segurança:', policyError);
      }
    }
  } catch (e) {
    console.error('Exceção ao criar tabela codigos_grupos_estudo:', e);
  }
}

// Função para obter grupos locais e sincronizá-los
