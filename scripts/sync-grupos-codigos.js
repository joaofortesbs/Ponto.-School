...
async function checkAndCreateTables() {
  ...
  try {
    ...
    if (gruposError && gruposError.code === '42P01') {
      ...
      try {
        // Chamar a API para criação de tabelas
        // Caso esteja executando localmente, usa o método RPC
        try {
          const response = await fetch(`${process.env.API_URL || 'http://localhost:3000'}/api/fix-tables`, {
            method: 'POST',
            headers: {
          ...
          if (response && response.ok) {
            ...
            throw new Error('API de correção de tabelas falhou');
          }
        } catch (apiError) {
          console.log('Não foi possível usar a API, tentando método direto...');
