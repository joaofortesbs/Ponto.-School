...
- **Core Features**:
    ...
    - **`decidir_atividades_criar` v2.0**: Production-hardened decision capability for activity creation with a 5-layer architecture including tolerant JSON parsing, system prompt separation with retry loops, soft validation and auto-repair, smart context-aware fallback, and pipeline/template field integrity.
