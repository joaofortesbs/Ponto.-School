...
- **AI Integration**: A unified, resilient multi-model cascade system (LLM Orchestrator v3.0 Enterprise) with a 5-tier architecture across 7 models, including circuit breakers, rate limiters, retry mechanisms, input sanitization, smart routing, and in-memory caching. The core AI agent, "Mente Orquestradora Architecture v7.0," features autonomous capability selection, a supreme unified context engine, `Context Engine v2.0`, `SessionStore` v2.0 with `InteractionLedger`, `ContextAssembler`, `ConversationCompactor`, `GoalReciter`, and **SmartRouter v1.0** (LLM-based, replaces regex IntentClassifier).
    - **SmartRouter v1.1 (Feb 2026)**: Cascade Pattern routing system (inspired by Genspark/MagicSchool). Architecture: (1) **Fast Rules Layer** (<1ms) catches ~70% of cases with keyword matching — calendar words → CAPABILITY_DIRETA, greetings → CONVERSAR, creation verbs + pedagogical content → EXECUTAR; (2) **LLM Layer** (<500ms) handles ambiguous cases only; (3) **Conflict Resolution** — when fast rules and LLM disagree, CAPABILITY_DIRETA always wins over EXECUTAR (prevents "crie compromisso" misrouting). Replaces v1.0's LLM-first approach where regex fallback dominated. Old IntentClassifier (intent-classifier.ts) is now fully deprecated — unused imports removed from orchestrator. Located in `context-engine/smart-router.ts`.
    - **handleDirectResponse (CONVERSAR path)**: Uses `buildContextForConversation()` which injects `SYSTEM_PROMPT_CONVERSAR` (conversational identity, NO anti-literalism) + recent conversation history + session ledger. Completely isolated from the execution-focused `SYSTEM_PROMPT` to prevent LLM confusion. `sendFollowUpMessage` is route-aware: uses `buildContextForConversation` when no active plan, `buildContextForFollowUp` (with full SYSTEM_PROMPT) when a plan is in progress.
    - **Route-Specific System Prompts (Feb 2026)**: `SYSTEM_PROMPT` (execution-focused, anti-literalism) used only for EXECUTAR/planner paths. `SYSTEM_PROMPT_CONVERSAR` (conversational, no anti-literalism) used only for CONVERSAR path. `buildContextForCapability` already uses `includeSystemPrompt: false`. This prevents the LLM from receiving contradictory instructions.
    - **CAPABILITY_DIRETA Fallback (Feb 2026)**: When a direct capability (like `gerenciar_calendario`) fails, the system now falls back to a conversational response via `handleDirectResponse` instead of creating a full EXECUTAR plan. This prevents calendar requests from generating activity cards on failure.
