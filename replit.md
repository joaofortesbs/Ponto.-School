...
- **Core Features**:
    ...
    - **Final Response Service v2.0 — Hybrid Deterministic + AI (Feb 2026)**: Complete refactoring of the final response generation for the EXECUTAR path. Architecture: (1) **Deterministic Phase Grouping** — code classifies every activity into 5 pedagogical phases (Engajamento, Conteúdo, Prática, Avaliação, Complementos) using a complete mapping of all 70+ activity types from `schoolPowerActivities.json` catalog. Classification uses 3-tier lookup: `TIPO_TO_PHASE` (direct tipo match) → `TITLE_TO_CATEGORY` (exact title from catalog) → fuzzy title match → fallback to 'pratica'. (2) **Deterministic Structure Builder** — code assembles the full response string with `[[FASE:]]`, `[[ATIVIDADE:]]`, `[[ARQUIVO:]]` markers positioned correctly. This NEVER fails regardless of LLM behavior. (3) **AI Strategic Paragraphs** — LLM generates ONLY: abertura (1-2 sentences), one strategic paragraph per phase (2-4 sentences with pedagogical insights), and encerramento (3-5 sentences with next steps). Prompt reduced from ~200 to ~50 lines, requesting JSON output. (4) **Triple Fallback** — if AI JSON parsing fails, tries regex extraction → narrative text extraction → deterministic fallback paragraphs. (5) **Clean Context** — sends only `tema_limpo`, `disciplina`, `turma`, and activity list to LLM instead of full execution context. Uses `systemPrompt` separation. For <=2 items, uses simple format without phases. Located in `context/final-response-service.ts`.
