...
- **Core Features**:
    ...
    - **Activity Version System**: Dual-version system categorizing activities into Interactive (fully functional UI) and Text Version (content delivered as formatted text). Interactive activities: `lista-exercicios`, `quiz-interativo`, `flash-cards`. Text version activities: `plano-aula`, `sequencia-didatica`, `tese-redacao`. Managed by `activityVersionConfig.ts` with `TextVersionGenerator.ts` for AI text generation and `ContentExtractModal.tsx` for content display.
