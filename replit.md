...
- **Core Features**:
    ...
    - **ActivityStorageContract**: Centralized helper module (`src/features/schoolpower/services/activity-storage-contract.ts`) providing `writeActivityContent()` and `readActivityContent()` as single source of truth for all localStorage persistence. Enforces format rules per activity type: quiz-interativo gets `{success, data, timestamp}` wrapper, lista-exercicios uses `saveExerciseListData()`, flash-cards and others use flat JSON. Replaces 5+ scattered localStorage write points with one canonical implementation. InlineActivitiesCard subscribes to `content-sync-update` events showing live loading spinner for heavy activities until generation completes.
