...
- **Core Features**:
    ...
    - **Agentic Artifacts System (CRIAR_ARQUIVO) v2.0**: First-class V2 capability registered under the CRIAR macro-category (pattern: CRIAR/criar_arquivo), appearing in the DevModeCard during execution with full observability. Generates 5 artifact types (`dossie_pedagogico`, `resumo_executivo`, `roteiro_aula`, `relatorio_progresso`, `guia_aplicacao`) via LLM with specialized prompts per type. Intelligent type detection based on context keywords. Architecture: Planner includes `criar_arquivo` as final pipeline step → Executor runs it as V2 capability with CapabilityInput/CapabilityOutput → criarArquivoV2 generates artifact → dispatches `artifact:generated` CustomEvent → ChatLayout listener → addArtifactCard to Zustand chatState → MessageStream renders minimalist ArtifactCard (file icon + title + subtitle, left-aligned) → click opens ArtifactViewModal. Key files: `src/features/schoolpower/agente-jota/capabilities/CRIAR_ARQUIVO/` (types, artifact-generator, criar-arquivo-v2, registry, index), `src/features/schoolpower/interface-chat-producao/components/ArtifactCard.tsx` (minimalist card: FileText icon left, title+subtitle right, no badges/stats/gradients), `src/features/schoolpower/interface-chat-producao/components/ArtifactViewModal.tsx` (Notion/Manus-inspired continuous document modal with Editor.js-style blocks), `src/features/schoolpower/interface-chat-producao/components/artifact-editorjs-converter.ts` (markdown-to-block converter with paragraph merging, list parsing, inline formatting). ArtifactViewModal features: dark background (#000822), header (#040b2a), Modal Geral-matching backdrop overlay (opacity 0.6, blur 4px), max-width 680px content column, inline horizontal mini-lines TOC with scroll-based active highlighting and click navigation, sequential staggered fade-in animation (framer-motion), serif fonts (Georgia/Palatino) for body text and sans-serif (Inter/SF Pro) for headers, contentEditable-based editing preserving exact font styling (no textarea), Notion-style drag-and-drop via @dnd-kit/sortable with GripVertical hover handle for all block types, list item and table cell edit persistence via dedicated handlers, scoped container queries for TOC navigation. Artifact failures are isolated and non-critical (do not break the main pipeline).
    - **ArtifactViewModal Animation System**: Exact replication of Modal Geral backdrop animation: ANIMATION_DURATION=150ms with cubic-bezier(0.4, 0, 0.2, 1), isVisible/isAnimating dual state management, double requestAnimationFrame for mount animation, setTimeout for delayed unmount, CSS-driven transitions (no AnimatePresence). Body overflow managed via isVisible state with proper cleanup.
    - **ArtifactViewModal InlineTOC Positioning**: Mini-lines TOC navigation absolutely positioned on the right side of the modal (right-4, top-1/2, -translate-y-1/2), outside the scroll container, with items-end alignment. Conditionally rendered when tocItems.length > 0.
    - **ArtifactViewModal Block Selection System**: Notion-style block selection via grip handle click (didDragRef distinguishes click from drag). Selected blocks get light blue highlight (rgba(56, 139, 253, 0.08)). BlockContextMenu component with framer-motion scale animation (0.85→1, 150ms), dark themed (#0a1128 bg, #1a2444 border), Portuguese labels (Duplicar, Mover para cima, Mover para baixo, Excluir). Escape key deselects block first then closes modal. Click-outside deselection via scroll container onClick. stopPropagation on grip and menu prevents unwanted triggers.
    - **ArtifactViewModal BlockOutline System**: Minimal line-only outline navigation on extreme right side (right: 6px). No text labels - only horizontal line indicators with varying widths by level. Active section: blue line (2px, #1D7AFC) with glow/pulse animation, 22px wide. Sub-level items: 10px, top-level: 16px. Items aligned to right (items-end). IntersectionObserver with 16ms debounce for 60fps scroll sync using rootMargin '-10% 0px -70% 0px' with multiple thresholds. Navigation uses getBoundingClientRect() for reliable scroll-to-section. Title tooltip on hover. Positioned absolutely, vertically centered, hidden on small screens (lg:flex).
    - **ArtifactViewModal FloatingToolbar System**: Notion-style floating inline formatting toolbar above text selection within contentEditable blocks. Includes "Turn Into" dropdown (TurnIntoMenu component) triggered by T button with ChevronUp indicator. Turn Into menu options: H1-H6 headings, Text (paragraph), List, Ordered List, Todo - with active item detection (blue color + checkmark). Block detection via data-block-id attribute on SortableBlock. Keyboard navigation: ArrowUp/Down to navigate menu, Enter to select, Escape to close. Block transformation via handleBlockTransform preserves text content across type changes. Format buttons: B (bold), I (italic), S (strikethrough), code (inline), link. Positioned using getBoundingClientRect() relative to scroll container with scrollTop offset. 50ms debounce on selectionchange. Dark themed (#1a1a2e bg, #141422 dropdown bg). Code formatting uses extractContents + appendChild pattern with try/catch.
    - **Todo Block Type**: Renders checkbox with editable text. Checkbox toggles checked state via handleTodoToggle. Checked items show strikethrough text styling. Supports contentEditable text editing alongside checkbox state.
