...
- **Core Features**:
    ...
    - **Mente Orquestradora Architecture v7.0**: Autonomous capability selection with supreme unified context engine. Key components:
      ...
      - **Orchestrator Integration v2.0**: orchestrator.ts uses ContextGateway for all context building, IntentClassifier for routing, SessionStore as single source of truth. MemoryManager maintained as read-only bridge for executor backward compat. No more dual-writes to ContextManager.
