...
- **Core Features**:
    ...
    - **AI Calendar Creation (criar_compromisso_calendario)**: Zero-click AI-to-calendar integration. Jota AI agent creates calendar appointments directly from chat (capability registered in CRIAR/ category with 13 aliases). Features: NLP date parsing (DD/MM and YYYY-MM-DD), automatic icon inference from title, time validation, `created_by: 'jota_ia'` audit trail. CustomEvent dispatch triggers instant calendar UI refresh. First educational platform with autonomous AI calendar creation.
