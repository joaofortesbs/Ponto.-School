...
- **Core Features**:
    ...
    - **Mente Orquestradora Architecture**: Replaced fixed 5-step pipeline with autonomous capability selection. Key changes: planning-prompt.ts uses multi-intent examples with broad/descriptive steps, planner.ts has intelligent fallback, executor.ts uses V2_REGISTRY Map + narrative generation + dynamic replanning, ChatLayout.tsx auto-executes plans and propagates narrative/replan events. Modular prompt system: system-prompt.ts (identity/personality), narrative-prompt.ts (inter-step text), replanning-prompt.ts (live plan adjustment). ProgressiveExecutionCard displays narrative text between ObjectiveCards and replan notices.
