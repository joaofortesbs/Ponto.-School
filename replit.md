...
- **Core Features**:
    ...
    - **AI Calendar Capability (criar_compromisso_calendario)**: Allows Jota AI to programmatically create calendar events/appointments. Supports batch creation, conflict detection, activity linking, and automatic UI updates via CustomEvents. Registered in V2_REGISTRY with 11+ aliases. Non-critical capability (failures don't halt pipeline). Visual feedback via CompromissoCalendarioCard in chat.
