...
- **Core Features**:
    ...
    - **Quiz Interativo Protection System (Jan 2026)**: Multi-layer validation to prevent user prompt leakage:
      - **Theme Extraction**: 3-tier priority system extracts actual theme from user objectives (quoted text → "sobre/tema" patterns → subject defaults)
      - **Prompt Detection**: `containsUserPromptPatterns()` filters questions containing instruction verbs (criar, fazer, gerar, preciso)
      - **Validation Methods**: `isValidQuestion()` and `sanitizeQuizQuestions()` at both generation and persistence layers
      - **Educational Fallback**: Subject-specific question banks for História, Matemática, Português, Ciências when AI fails
      - **Interface Compatibility**: Updated TypeScript interfaces accept alternative AI field names (texto, alternativas, resposta_correta, feedback)
      - **Data Flow**: gerar_conteudo_atividades → QuizInterativoGenerator (sanitize) → autoBuildService (sanitize) → QuizInterativoPreview
