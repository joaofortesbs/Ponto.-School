...
- **Core Features**:
    ...
    - **Agentic Artifacts System (CRIAR_ARQUIVO)**: Comprehensive file/artifact generation capability that produces structured complementary materials after AI plan execution. Generates 5 artifact types (`dossie_pedagogico`, `resumo_executivo`, `roteiro_aula`, `relatorio_progresso`, `guia_aplicacao`) via LLM with specialized prompts per type. Intelligent type detection based on context keywords from the session. Architecture: Orchestrator → generateArtifact (after 3rd call/final response) → CustomEvent('artifact:generated') → ChatLayout event listener → addArtifactCard to Zustand chatState → MessageStream renders ArtifactCard → click opens ArtifactViewModal. Key files: `src/features/schoolpower/agente-jota/capabilities/CRIAR_ARQUIVO/` (types, artifact-generator, index), `src/features/schoolpower/interface-chat-producao/components/ArtifactCard.tsx` (glassmorphism card with color-coding, stats, animated entrance), `src/features/schoolpower/interface-chat-producao/components/ArtifactViewModal.tsx` (full-screen modal with tabbed sidebar navigation, section rendering, copy functionality). Inspired by Manus AI's file-as-proof philosophy, Kimi's specialized agents, and Claude's artifact sidebar. Artifact failures are isolated and do not break the main chat flow.
