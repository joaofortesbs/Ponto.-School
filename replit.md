...
- **AI Integration**: A unified, resilient multi-model cascade system (LLM Orchestrator v3.0 Enterprise) with a 5-tier architecture across 7 models, including circuit breakers, rate limiters, retry mechanisms, input sanitization, smart routing, and in-memory caching. The core AI agent, "Mente Orquestradora Architecture v8.0," features:
    - **Unified Router v1.0**: Single LLM call that replaces IntentClassifier (regex) + Planner + InitialResponse (3 calls → 1). Classifies intent into chat/execute/quick_action modes, generates execution plan, and creates initial response simultaneously. Located in `unified-router.ts` with prompt in `prompts/unified-router-prompt.ts`.
    - **MenteMaior v2.0 (Decision Power)**: Enhanced inner monologue with micro-decision capability. After each step execution, MenteMaior evaluates ALL remaining steps and returns step-level decisions: `keep` (proceed as planned), `skip` (step became unnecessary), or `adapt` (modify parameters based on results). This enables dynamic plan adaptation during execution without full replanning.
    - **Context Engine v2.0**: `SessionStore` v2.0 with `InteractionLedger`, `ContextAssembler`, `ConversationCompactor`, `GoalReciter`.
    - **Backward Compatibility**: Falls back to legacy `createExecutionPlan()` if Unified Router fails. Post-validations (salvar_atividades_bd injection, calendar detection) still applied.
