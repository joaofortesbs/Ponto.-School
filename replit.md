...
- **Core Features**:
    ...
    - **ArtifactViewModal BlockOutline System**: Notion-style hierarchical block outline on right side showing heading text (truncated to 28 chars) with gray horizontal lines. Active section highlighted with blue line (2px, #1D7AFC) with glow/pulse animation. Text labels hidden by default, revealed on nav hover (data-outline-active attribute preserves active item's blue color). Sub-level items indented (12px). IntersectionObserver with 16ms debounce for 60fps scroll sync using rootMargin '-10% 0px -70% 0px' with multiple thresholds. Fallback to manual rect calculation when no items in intersection. Positioned absolutely right side, vertically centered, hidden on small screens (lg:block). Replaces old InlineTOC mini-lines component.
    - **ArtifactViewModal FloatingToolbar System**: Notion-style floating inline formatting toolbar appearing above text selection within contentEditable blocks. Format buttons: T (removeFormat), B (bold), I (italic), S (strikethrough), code (inline <code> element with Notion styling), link (prompt URL). Positioned using getBoundingClientRect() relative to scroll container with scrollTop offset, centered above selection with overflow protection. 50ms debounce on selectionchange, validates selection within contentEditable inside container. Active format detection via queryCommandState. Scale animation 0.95→1 (150ms, framer-motion). preventDefault on mouseDown preserves selection. Dark themed (#1a1a2e bg, #2a2a4a border). Code formatting uses extractContents + appendChild pattern with try/catch for multi-node edge cases.
