...
- **Core Features**:
    - **School Power Tab System v1.0** (Mar 2026): Browser-tab-style session history for School Power. Each conversation becomes a tab that visually emerges from the card's top border — active tab fuses seamlessly with the card border (active: `border-b-0 mb-[-1px]`, concave corner shadows). Architecture: `SchoolPowerTabBar.tsx` (visual component with AnimatePresence, Lucide icons, concave pseudo-element corners), `useTabsStore.ts` (Zustand + `localStorage` `school-power-tabs`, max 6 tabs, `saveTabState`/`openNewTab`/`closeTab`/`reorderTabs`), `useSchoolPowerTabs.ts` (integration hook that snapshots and restores `useChatState` messages + `useSchoolPowerFlow` flowState/flowData + `useChosenActivitiesStore` activities when switching tabs), `types.ts` (`TabSnapshot`, `TabBarTab`, `TabIcon` interfaces). Layout restructured: wrapper div (no border) → TabBar (37px, sits above card) → card div (`flex-1`, border). Tab titles update to first 22 chars of first message; icons change: `home` → `chat` (on first message) → `activity` (when activities are generated). `useSchoolPowerFlow` exposes `restoreState(state, data)` for tab restoration. Pending: FASE 3 (backend persistence) awaiting user approval.
