...
- **Core Features**:
    ...
    - **`decidir_atividades_criar` v3.0**: Production-hardened decision capability for activity creation with a 7-layer architecture: Layer 0 (Gemini Function Calling with schema-enforced structured output), Layer 1 (tolerant JSON parsing with key normalization), Layer 2 (system prompt separation with retry loops + contamination detection), Layer 3 (soft validation + auto-repair), Layer 4 (smart context-aware keyword fallback), Layer 5 (pipeline/template field integrity), and a decision-aware local fallback that generates valid JSON decision structures when all APIs are unavailable.
