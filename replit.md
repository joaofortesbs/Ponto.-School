...
- **Core Features**:
    ...
    - **Powers System v3.2 ENTERPRISE (Feb 2026)**: Virtual currency for AI capabilities with per-action pricing and enterprise-grade bidirectional synchronization between localStorage and Neon DB. Features daily allowance, pending transactions queue, polling system, synchronous charges, event-based decoupling (no circular dependencies), lazy polling initialization, auto-initialization on email event, multiple email propagation paths (localStorage cache, event listener, PerfilCabecalho direct call, refreshProfileInBackground), and guard against duplicate listeners.
