...
## Recent Changes
- **2026-02-10**: Text Activity Pipeline Fix (MACROs 1-7) — Complete end-to-end fix for `atividade-textual` data flow: (1) Added 12-field mapping in gerar-conteudo-atividades.ts so textContent/sections populate correctly. (2) Content Envelope in SALVAR_BD with polymorphic structure (content_type: 'text_activity', text_payload, schema_version 2.0). (3) Validation fix to skip empty campos check for text activities. (4) ConstructionGrid.tsx and ChatLayout.tsx redirect atividade-textual to ArtifactViewModal (not ActivityViewModal) with ConstructionActivity→ArtifactData conversion. (5) Removed atividade_textual from CRIAR_ARQUIVO normalizeArtifactType to prevent routing collision. (6) EditActivityModal shows rich textContent in dark-themed Textarea for text activities. (7) TextVersionGenerator retry strategy with output validation (textContent ≥50 chars, sections >0, retry with format-reinforced prompt).
