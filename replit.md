...
- **Core Features**:
    ...
    - **Mente Orquestradora Architecture v6.0**: Autonomous capability selection with unified context engine. Key components:
      - **Context Engine** (`agente-jota/context-engine/`): 3-layer architecture (static instructions, compressed session history, dynamic call-specific data) inspired by Manus AI, Kimi Agent, and Devin. Components: `ContextAssembler` (optimizes context per call type), `ConversationCompactor` (intelligent summarization, not truncation), `GoalReciter` (prevents context drift), `SessionStore` (unified state management replacing fragmented MemoryManager+ContextManager).
      - **MenteMaior** (`mente-maior.ts`): Unified inner monologue using ReAct pattern — replaces 2 separate AI calls (generateNarrativeForStep + checkReplanning) with single call returning `{narrative, replan}`. Reduces AI calls from 12-18 per request to fewer.
      - **Executor Integration**: executor.ts uses MenteMaior for step reflection, emits narrative/replan events via progress callback. `buildMinimalSession` fallback with telemetry if SessionStore is empty.
      - **Orchestrator Integration**: orchestrator.ts creates sessions, registers conversation turns, sets plans, and cleans up via ContextEngine. Legacy MemoryManager/ContextManager maintained as bridge during migration.
      - **UI**: ChatLayout.tsx auto-executes plans, ProgressiveExecutionCard displays narrative text between ObjectiveCards and replan notices.
