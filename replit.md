...
- **Core Features**:
    ...
    - **AI Calendar Creation (criar_compromisso_calendario) v2.0 Batch**: Zero-click AI-to-calendar integration with batch support. Jota AI agent creates single or multiple calendar appointments directly from chat. Features: batch creation via `/api/calendar/events/batch`, orchestrator AI generates complete event params (titulo, data, hora, icone, labels), auto-generation from created activities (`vincular_atividades: true`), intelligent weekday distribution with school hours (07:30-15:50), subject label inference (Matemática, Português, Ciências, etc.) with color coding, activity linking via `linked_activities` jsonb field. NLP multi-date parsing, `created_by: 'jota_ia'` audit trail. CustomEvent `calendar-events-updated` triggers instant UI refresh. Competitor-aligned architecture (Motion/Reclaim.ai/Google Calendar pattern).
