...
- **Core Features**:
    ...
    - **Mente Orquestradora Architecture v7.0**: Autonomous capability selection with supreme unified context engine. Key components:
      ...
      - **Structured Response System**: After execution completes, the orchestrator collects created items (activities + artifacts) via `executor.getCollectedItems()`. The final response prompt instructs the AI to use `[[ATIVIDADES]]` and `[[ARQUIVO:titulo]]` markers. `structured-response-parser.ts` converts the text + markers into blocks (text, activities_card, artifact_card). `StructuredResponseMessage.tsx` renders these blocks inline with interactive cards. Falls back to appending cards at the end if AI doesn't use markers. Artifact deduplication prevents duplicates between collected items and pending artifacts.
    ...
    - **ActivityStorageContract**: Centralized helper module (`src/features/schoolpower/services/activity-storage-contract.ts`) providing `writeActivityContent()` and `readActivityContent()` as single source of truth for all localStorage persistence. Enforces format rules per activity type: quiz-interativo gets `{success, data, timestamp}` wrapper, lista-exercicios uses `saveExerciseListData()`, flash-cards and others use flat JSON. Replaces 5+ scattered localStorage write points with one canonical implementation.
    - **Activity Content Sync (DB Fallback)**: When opening activities from chat's structured response card, `handleOpenActivity` in ChatLayout tries local sources first (ContentSync, StorageContract, localStorage), then falls back to DB via `/api/atividades-neon/by-original-id/:id` (with `::jsonb` cast for text column) and `/api/atividades-neon/by-type-user/:userId/:tipo`. Resolves ID mismatch between `built-*` temporary IDs and 8-char DB IDs.
    - **InlineActivitiesCard (Redesigned)**: Stateless component with no loading gates — activities are instantly clickable. Conditional header ("Suas X atividades:") only shown for 2+ activities. Uses Lucide icons per activity type (Brain, Layers, Search, etc.). Background `#040b2a`, max-width 340px. Shows real activity description as subtitle via `descricao` field populated from executor's `getCollectedItems()`.
