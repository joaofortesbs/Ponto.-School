...
- **Core Features**:
    ...
    - **Ponto. Flow (Artifact System v2.0)**: Generic file/artifact generation capability (`criar_arquivo`) within Agente Jota's CRIAR category, located at `src/features/schoolpower/agente-jota/capabilities/CRIAR_ARQUIVO/` (capability) and `src/features/schoolpower/interface-chat-producao/artifact-system/` (UI). After plan execution and final response (3rd call), the orchestrator automatically triggers `criarArquivo()` using ContextManager context (not DossieStore). Generates structured text documents via AI (executeWithCascadeFallback) with professional fallback content. Supports multiple artifact types: dossie, resumo, roadmap, relatorio, generico. Architecture: Orchestrator.executeAgentPlanWithDetails → criarArquivo (AI generation with ContextManager.gerarContextoParaChamada('final')) → artifactData returned → ChatLayout dispatches addArtifactCard → MessageStream renders ArtifactCard (glassmorphism clickable card) → ArtifactViewModal (tabbed viewer with dynamic sections, copy-to-clipboard). Sections include: Resumo Executivo, Roteiro de Aplicação (timeline), Ganchos de Atenção, Mensagem para Pais (WhatsApp), Coordenação, Estratégia Pedagógica. Non-blocking: failures don't affect main execution flow. Old dossie_card messages still render via legacy DossieCard for backward compatibility.
