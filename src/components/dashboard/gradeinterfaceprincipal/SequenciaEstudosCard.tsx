...
export default function SequenciaEstudosCard() {
  ...
  const iniciarCronometro = () => {
    ...
    timerRef.current = setInterval(() => {
      ...
      // Se chegou à meia-noite, permite novo check-in e mostra interface
      if (tempo.totalMs <= 0) {
        if (timerRef.current) {
          clearInterval(timerRef.current);
        ...
        if (ultimoCheckIn) {
          ...
          // Se o último check-in não foi no dia anterior (quebrou sequência)
          if (ultimaDataStr !== ontemStr) {
            // Resetar a sequência se passou um dia inteiro sem check-in
            const novosDados: StreakData = {
              diasConsecutivos: 0,
              recordeDias,
              diasParaProximoNivel: 3,
              ...
              proximaRecompensa: "Badge Iniciante",
              ultimoCheckIn: null,
              eventos: streakEvents
            };

            setDiasConsecutivos(0);
            setDiasParaProximoNivel(3);
            setProximaRecompensa("Badge Iniciante");

            // Salva os novos dados com a sequência resetada
            salvarDadosSequencia(novosDados);
          }
        }
      }
    }, 1000);
            ...
            <div className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-semibold 
              ${tempoRestante.horas === 0 && tempoRestante.minutos === 0 && tempoRestante.segundos === 0
                ? isLightMode 
                  ...
                  : 'bg-[#FF6B00]/30 text-[#FF6B00] animate-pulse'
                : isLightMode 
                  ? 'bg-gray-100 text-gray-700' 
                  : 'bg-[#14253d] text-gray-300'}`}>
              <Clock className={`h-3.5 w-3.5 ${
                tempoRestante.horas === 0 && tempoRestante.minutos === 0 && tempoRestante.segundos === 0
                ? 'text-[#FF6B00] animate-bounce' 
                : 'text-[#FF6B00]'
              }`} />
              <span>
