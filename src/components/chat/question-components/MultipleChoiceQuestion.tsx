    ...
    return selectedQuestion.options.map((option: any, idx: number) => {
      ...
      return (
        <div 
          key={option.id}
          className={`flex items-center space-x-2 p-2 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 ${
            isSelected ? 'bg-blue-50 dark:bg-blue-900/20 border border-blue-300 dark:border-blue-700' : ''
          } ${
            selectedOption && isCorrect ? 'bg-green-50 dark:bg-green-900/20 border border-green-300 dark:border-green-700' : ''
          }`}
