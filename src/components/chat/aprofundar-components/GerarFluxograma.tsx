                    ...
                    if (!exportMenu) {
                      ...
                      exportMenu.innerHTML = `
                        <div class="flex flex-col space-y-1">
                          <button class="text-left px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors flex items-center text-gray-700 dark:text-gray-300" onClick="(function(){
                          const flowWrapper = document.querySelector('.react-flow');
                          if (!flowWrapper) {
                            alert('Não foi possível encontrar o fluxograma para exportar.');
                            return;
                          ...
                          import('html-to-image').then(htmlToImage => {
                            ...
                            // Exportar como PNG
                            htmlToImage.toPng(flowWrapper, options)
                              .then(dataUrl => {
                                const link = document.createElement('a');
                                link.download = 'fluxograma.png';
                                link.href = dataUrl;
