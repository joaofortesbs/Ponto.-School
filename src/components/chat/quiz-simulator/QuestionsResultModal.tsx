        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
          <div className="grid grid-cols-1 gap-4">
            {questions.map((question, index) => {
              let tagColorClass = "bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300";
              
              if (question.type.toLowerCase().includes("múltipla") || question.type.toLowerCase().includes("multipla")) {
                tagColorClass = "bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300";
              } else if (question.type.toLowerCase().includes("discursiva")) {
                tagColorClass = "bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300";
              } else if (question.type.toLowerCase().includes("v/f") || question.type.toLowerCase().includes("verdadeiro") || question.type.toLowerCase().includes("falso")) {
                tagColorClass = "bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300";
              }
              ...
              return (
                <div key={index} className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:shadow-md transition-all">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className={`text-sm px-2 py-0.5 rounded-md ${tagColorClass}`}>
                      {question.type}
                    </span>
                  </div>
