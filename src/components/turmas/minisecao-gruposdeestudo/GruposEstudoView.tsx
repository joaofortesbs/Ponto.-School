...
export default function GruposEstudoView() {
  ...
  // Função para acessar grupo
  const handleAccessGroup = async (group: Group) => {
    try {
      console.log('Tentando acessar grupo:', group.id);
      ...
      console.log('Verificação de membro:', membership, membershipError);
      ...
      if (membershipError) {
        if (membershipError.code === 'PGRST116') {
          ...
          toast({
            title: "Erro",
            description: "Erro ao verificar membresia. Tente novamente.",
            variant: "destructive"
          });
      ...
      console.log('Membresia confirmada, exibindo interface do grupo');
      setSelectedGroup(group);

    } catch (error: any) {
      console.error('Erro geral ao acessar grupo:', error);
      toast({
        title: "Erro",
        description: "Ocorreu um erro ao tentar acessar o grupo.",
        variant: "destructive"
      });
    }
  };
  ...
  // Renderizar interface do grupo ou lista de grupos
  if (selectedGroup) {
    return (
