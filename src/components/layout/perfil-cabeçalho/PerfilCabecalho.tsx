...
const PerfilCabecalho: React.FC = () => {
  const { profile, powers, isLoading } = useProfile();
  const [isDark, setIsDark] = useState(false);
  const [profileImage, setProfileImage] = useState<string | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  useEffect(() => {
    console.log('[PerfilCabecalho] Profile:', profile?.nome_completo);
    console.log('[PerfilCabecalho] Powers:', powers);
  }, [profile, powers]);
  ...
  const displayName = profile?.nome_completo || profile?.nome_usuario || profile?.display_name || profile?.username || "Usuário";
  const avatarSeed = profile?.nome_usuario || profile?.username || "user";
  const avatarUrl = profileImage || profile?.imagem_avatar || profile?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${avatarSeed}`;
        ...
        <div className="flex items-center gap-1.5 px-1.5 py-1 rounded-full bg-white dark:bg-[#0A2540] border border-[#E0E1DD] dark:border-white/10 hover:border-[#FF6B00]/30 dark:hover:border-[#FF6B00]/30 transition-all duration-300 cursor-pointer group shadow-sm hover:shadow-md">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <span className="text-sm font-semibold text-[#FF6B00]">
              {powers !== null ? powers : (isLoading ? '...' : '0')}
            </span>
