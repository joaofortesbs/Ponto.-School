...
export default function ChatInteligente() {
  ...
  const [showTurboSection, setShowTurboSection] = useState(false);

  const handleTurboClick = () => {
    setShowTurboSection(true);
  };
    ...
    <div className="h-full flex flex-col">
      ...
      {showTurboSection ? (
        <div className="flex-1 flex items-center justify-center">
          <p className={`text-2xl font-medium ${theme === "dark" ? "text-white" : "text-gray-900"}`}>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <EpictusTurboCard onTurboClick={handleTurboClick} />
            <EpictusIACard />
            <TutorInteligenteCard />
            <BrainstormCard />
          </div>
