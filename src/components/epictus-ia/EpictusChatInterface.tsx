...
const EpictusChatInterface: React.FC = () => {
  ...
  const [notebookMode, setNotebookMode] = useState(false); // Added state for notebook mode
  ...
  const handleNotebookSimulation = (messageContent: string) => {
    //Simulate Notebook here. This is a placeholder.  Replace with actual notebook generation logic.
    const notebookContent = simulateNotebook(messageContent);
    setNotebookMode(true); //Activate Notebook Mode
    //Update UI to display notebookContent
    //Example: setNotebookContent(notebookContent)
  };


  const simulateNotebook = (messageContent: string): string => {
    //This is a placeholder function.  Replace with your actual notebook simulation logic.
    //This function should take the messageContent and transform it into a notebook-style summary.

    //Example:  Very basic placeholder. Replace with actual logic to follow prompt instructions.
          ...
          >
            ...
            >
              ...
              <MessageToolsDropdown 
                ...
                onNotebookSimulation={handleNotebookSimulation} // Added prop for notebook simulation
              />
            </div>
...
// Placeholder for MessageToolsDropdown component.  Needs to be implemented separately.
const MessageToolsDropdown: React.FC<{ align: string; messageId: string; messageContent: string; onNotebookSimulation: (content: string) => void }> = ({ align, messageId, messageContent, onNotebookSimulation}) => {
  return (
    <div>
      <button onClick={() => onNotebookSimulation(messageContent)}>Simular Caderno</button>
    </div>
  );
};