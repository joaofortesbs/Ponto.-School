...
export async function gerarConteudoAtividadesV2(
  ...
  try {
    // 1. OBTER ATIVIDADES DECIDIDAS DA CAPABILITY ANTERIOR
    const decidirResult = input.previous_results?.get('decidir_atividades_criar');
    
    let activities: ChosenActivity[] = [];
    
    if (decidirResult?.success && decidirResult.data?.chosen_activities) {
      activities = decidirResult.data.chosen_activities;
      debug_log.push({
        timestamp: new Date().toISOString(),
        type: 'info',
        narrative: `Recebi ${activities.length} atividades para gerar conteúdo da capability "decidir_atividades_criar".`,
        technical_data: { source: 'capabilityResultsMap', count: activities.length }
      });
    } else {
      // Fallback para o store se não vier via Map (para robustez)
      activities = useChosenActivitiesStore.getState().getChosenActivities();
      debug_log.push({
        timestamp: new Date().toISOString(),
        type: 'warning',
        narrative: `Resultado de decisão não encontrado no Map. Usando fallback do ChosenActivitiesStore (${activities.length} atividades).`,
        technical_data: { source: 'store_fallback', count: activities.length }
      });
    }

    if (activities.length === 0) {
      return {
        success: true, // Sucesso vazio é válido se não há o que gerar
        capability_id: 'gerar_conteudo_atividades',
        execution_id: input.execution_id,
        timestamp: new Date().toISOString(),
        data: { generated_content: [], generated_count: 0, total_fields_filled: 0 },
        error: null,
        debug_log: [{
          timestamp: new Date().toISOString(),
          type: 'info',
          narrative: 'Nenhuma atividade pendente de conteúdo encontrada.'
        }],
        metadata: { duration_ms: Date.now() - startTime, retry_count: 0, data_source: 'none' }
      };
    }

    // 2. EXECUTAR GERAÇÃO PARA CADA ATIVIDADE
    ...
    for (const activity of activities) {
      const result = await generateContentForActivity(
        activity,
        input.context.conversation_context || '',
        input.context.user_objective || '',
        undefined, // onProgress opcional aqui
        'gerar_conteudo_atividades',
        'Gerando conteúdo para as atividades'
      );
      
      if (result.success) {
        results.push(result);
        const fieldCount = Object.keys(result.generated_fields).length;
        totalFieldsFilled += fieldCount;
        
        // ATUALIZAR O STORE IMEDIATAMENTE PARA REFLETIR NA UI
        useChosenActivitiesStore.getState().updateActivityFields(activity.id, result.generated_fields);
        
        debug_log.push({
          timestamp: new Date().toISOString(),
          type: 'discovery',
          narrative: `Conteúdo gerado para "${activity.titulo}": ${fieldCount} campos preenchidos.`,
          technical_data: { activity_id: activity.id, fields: Object.keys(result.generated_fields) }
        });
      } else {
        debug_log.push({
          timestamp: new Date().toISOString(),
    ...
    return {
      ...
      metadata: {
        ...
        retry_count: 0,
        data_source: 'multi_agent_generation'
      }
    };
    ...
    return {
      ...
      error: {
        ...
        message: `Erro no pipeline de geração: ${errorMessage}`,
        severity: 'high',
        recoverable: true
      },
      debug_log: [{
        timestamp: new Date().toISOString(),
        type: 'error',
        narrative: `Erro fatal no pipeline: ${errorMessage}`
      }],
      metadata: { duration_ms: Date.now() - startTime, retry_count: 0, data_source: 'error' }
    };
  }
}
