...
interface GeminiActivityResponse {
  ...
  isProcessed?: boolean; // Adicionado para indicar se a atividade foi processada
  cardContent?: { // Adicionado para o conteúdo do quadro interativo
    title: string;
    text: string;
  };
}
...
export async function generatePersonalizedPlan(
  ...
  try {
    ...
    const actionPlanItems = await Promise.all(validatedActivities.map(async (activityData) => {
        ...
        // Processar atividades específicas
        if (activityData.id === 'quadro-interativo') {
          console.log('🖥️ Processando Quadro Interativo...');
          try {
            // Assuming prepareQuadroInterativoData is defined elsewhere and imported
            // For this example to be runnable, this function would need to be provided.
            // Example structure:
            // async function prepareQuadroInterativoData({ respostasIA, customFields }: { respostasIA: any, customFields: Record<string, string> }): Promise<any> {
            //   // ... processing logic ...
            //   return { cardContent: { title: 'Processed Title', text: 'Processed Text' } };
            // }
            const quadroData = await prepareQuadroInterativoData({
              respostasIA: { data: activityData }, // Pass the current activityData
              customFields: activityData.customFields || {}
            });

            processedActivity = {
              ...processedActivity, // Spread existing properties
              ...quadroData, // Spread properties from quadroData
              isProcessed: true,
              cardContent: quadroData.cardContent || {
                title: activityData.personalizedTitle || activityData.title || 'Quadro Interativo',
                text: activityData.personalizedDescription || activityData.description || 'Conteúdo educativo interativo'
              }
            };
          } catch (error) {
          ...
          // For other activities, mark as processed if not 'quadro-interativo'
          processedActivity.isProcessed = true;
        }
    ...
    console.log(`✅ Total validated activities generated: ${actionPlanItems.length}`);

    // Converts to ActionPlanItems
    const finalActionPlanItems = convertToActionPlanItems(actionPlanItems, schoolPowerActivities);

    console.log('✅ Personalized plan generated successfully:', finalActionPlanItems);
    return finalActionPlanItems;
