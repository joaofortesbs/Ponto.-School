...
import { executeWithCascadeFallback } from './controle-APIs-gerais-school-power';
...
/**
 * Calls API with cascade fallback system
 * Uses the new multi-API controller for guaranteed responses
 */
async function callGroqAPI(prompt: string, uploadedFiles?: UploadedFile[]): Promise<string> {
  console.log('📡 Calling API with cascade fallback system...');
  ...
  console.log('🎯 Using cascade fallback system for guaranteed response...');
  
  const result = await executeWithCascadeFallback(fullPrompt, {
    onProgress: (status) => {
  ...
  if (!result.success || !result.data) {
    console.error('❌ Cascade fallback failed completely');
    throw new Error('All API attempts failed');
  }

  console.log(`✅ Response received from ${result.modelUsed} (${result.providerUsed})`);
  console.log(`📊 Total attempts: ${result.attemptsMade}, Latency: ${result.totalLatency}ms`);
  
  if (result.errors.length > 0) {
    console.warn('⚠️ Some models failed:', result.errors);
  }
