...
// Componente do Modal de Acesso Vitalício com renderização corrigida
const AcessoVitalicioModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  // Reset completo quando o modal fecha
  useEffect(() => {
    if (!isOpen) {
      setIsVisible(false);
      setData([]);
    ...
    // Animação progressiva dos dados do gráfico
    const dataTimer = setTimeout(() => {
      let currentIndex = 0;
      const interval = setInterval(() => {
        if (currentIndex < initialData.length) {
          setData(prev => [...prev, initialData[currentIndex]]);
          currentIndex++;
        } else {
  ...
  // Reset da visibilidade quando alterna entre interfaces
  useEffect(() => {
  ...
  // Interface da seleção de planos (estrutura corrigida)
  if (showPlanSelection) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
      ...
      >
        <motion.div
          ...
          className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6 max-w-md w-full mx-auto max-h-[90vh] overflow-y-auto"
        ...
        >
          ...
          <div className="space-y-4 mb-6">
            {[
              ...
              >
                <div className="w-5 h-5 rounded-full bg-gradient-to-r from-orange-500 to-orange-400 flex items-center justify-center shadow-sm flex-shrink-0">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 px-4 py-3 border-2 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors font-medium"
            >
              Voltar
            </button>
            <button
              ...
              className="flex-1 px-4 py-3 bg-gradient-to-r from-[#FF6B00] to-[#FF8A39] hover:from-[#E55A00] hover:to-[#FF7A29] text-white font-bold rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
            >
              Assinar Agora
            </button>
          </div>
  ...
  // Interface principal do modal (gráfico + benefícios)
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      <motion.div
        ...
        exit={{ opacity: 0, scale: 0.9, y: 20 }}
        transition={{ duration: 0.3, ease: "easeOut" }}
        className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6 max-w-2xl w-full mx-auto max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        ...
        >
          ...
          <div className="h-48 w-full relative">
            <svg className="w-full h-full" viewBox="0 0 400 200" preserveAspectRatio="none">
              ...
              <rect width="100%" height="100%" fill="url(#grid)" opacity={isVisible ? 0.3 : 0} style={{ transition: 'opacity 1s ease-out' }} />
            ...
            {data.length >= 1 && (
              <div 
                className="absolute text-xs font-semibold text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-800 px-2 py-1 rounded-lg shadow-md transform -translate-x-1/2 -translate-y-8"
                style={{
                  left: '8%',
                  top: '70%'
                }}
          ...
          <div className="space-y-4">
            {[
              ...
              <motion.div 
                ...
                className="flex items-center space-x-3"
              >
                <div className={`w-5 h-5 rounded-full flex items-center justify-center shadow-sm flex-shrink-0 ${
                  item.type === 'positive' 
