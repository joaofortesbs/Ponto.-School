...
export const EditActivityModal: React.FC<EditActivityModalProps> = ({
  ...
  useEffect(() => {
    if (activity && isOpen) {
      ...
      // Verificar se há conteúdo auto-gerado disponível
      const controller = AutomationController.getInstance();
      const autoGeneratedContent = controller.getGeneratedContent(activity.id);
      
      if (autoGeneratedContent && !generatedContent) {
        console.log('🤖 Conteúdo auto-gerado encontrado, carregando...');
        // Simular que o conteúdo foi gerado para exibir na pré-visualização
        if (loadSavedContent) {
          loadSavedContent();
        }
      }
