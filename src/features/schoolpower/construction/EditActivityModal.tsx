...
const EditActivityModal = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // SISTEMA INTEGRADO DE SINCRONIZAÇÃO COM BANCO NEON
      const profile = await profileService.getCurrentUserProfile();
      if (profile?.id) {
        console.log('🏦 Iniciando sincronização automática com banco Neon...');

        try {
          // Preparar dados estruturados para o banco
          const activityDataForNeon = {
            user_id: profile.id,
            codigo_unico: activitiesApi.generateUniqueCode(),
            tipo: activity.id,
            titulo: finalActivityData.title || finalActivityData.titulo || `Atividade ${activity.title}`,
            descricao: finalActivityData.description || finalActivityData.descricao || finalActivityData.title || activity.description,
            conteudo: {
              ...
              ...(activity.id === 'quiz-interativo' && quizInterativoContent && {
                questions: quizInterativoContent.questions,
                totalQuestions: quizInterativoContent.totalQuestions,
                timePerQuestion: quizInterativoContent.timePerQuestion
              }),
              ...(activity.id === 'flash-cards' && flashCardsContent && {
                cards: flashCardsContent.cards,
                totalCards: flashCardsContent.totalCards
              }),
              // Dados de sincronização
              syncMetadata: {
                version: '2.0',
                syncedAt: new Date().toISOString(),
                source: 'construction-interface'
              }
            }
          };

          console.log('📋 Dados preparados para banco Neon:', activityDataForNeon);
          ...
          if (existingActivities.success && existingActivities.data) {
            ...
            if (existingActivity) {
              ...
              console.log('🔄 Atualizando atividade existente no banco...');
              saveResult = await activitiesApi.updateActivity(existingActivity.codigo_unico, {
                titulo: activityDataForNeon.titulo,
                descricao: activityDataForNeon.descricao,
                conteudo: activityDataForNeon.conteudo
              });
            } else {
              // Criar nova atividade
              console.log('📝 Criando nova atividade no banco...');
              saveResult = await activitiesApi.createActivity(activityDataForNeon);
            }
          } else {
            // Criar nova atividade (primeira vez)
            console.log('📝 Criando primeira atividade no banco...');
            saveResult = await activitiesApi.createActivity(activityDataForNeon);
          }

          if (saveResult?.success) {
            console.log('✅ Atividade sincronizada com banco Neon com sucesso!');
            console.log('📋 Dados salvos:', saveResult.data);

            // Marcar no localStorage como sincronizada
            const constructedActivities = JSON.parse(localStorage.getItem('constructedActivities') || '{}');
            constructedActivities[activity.id] = {
              ...constructedActivities[activity.id],
              syncedToNeon: true,
              neonActivityId: saveResult.data.codigo_unico || saveResult.data.id,
            ...
            window.dispatchEvent(new CustomEvent('activity-synced-to-neon', {
              detail: {
                activityId: activity.id,
                neonId: saveResult.data.codigo_unico || saveResult.data.id,
                success: true
              }
            }));
            ...
            toast({
              ...
              description: "Sua atividade foi salva com sucesso no banco de dados.",
            });

          } else {
            console.warn('⚠️ Falha ao salvar no banco Neon:', saveResult?.error);

            // Marcar erro no localStorage
            const constructedActivities = JSON.parse(localStorage.getItem('constructedActivities') || '{}');
            constructedActivities[activity.id] = {
              ...constructedActivities[activity.id],
              syncedToNeon: false,
              syncError: saveResult?.error || 'Erro desconhecido',
            ...
            toast({
              title: "Aviso",
              description: "Atividade salva localmente. Será sincronizada automaticamente.",
              variant: "destructive",
            });
          }
        } catch (error) {
          console.error('❌ Erro crítico ao sincronizar com banco Neon:', error);

          // Marcar erro crítico no localStorage
          const constructedActivities = JSON.parse(localStorage.getItem('constructedActivities') || '{}');
          constructedActivities[activity.id] = {
            ...constructedActivities[activity.id],
            syncedToNeon: false,
            ...
            lastSyncAttempt: new Date().toISOString(),
            syncStatus: 'critical_error'
          };
          localStorage.setItem('constructedActivities', JSON.stringify(constructedActivities));

          toast({
            title: "Erro de Sincronização",
            description: "Falha ao salvar no banco. Tentativa automática em breve.",
            variant: "destructive",
          });
        }
      } else {
            ...
            if (activity?.id === 'plano-aula') {
              ...
              enrichedFormData = {
                ...
                subject: consolidatedCustomFields['Componente Curricular'] ||
                         ...
                         customFields['Disciplina'] ||
                         'Matemática',
                theme: consolidatedCustomCustomFields['Tema ou Tópico Central'] ||
                ...
                schoolYear: consolidatedCustomFields['Ano/Série Escolar'] ||
                           ...
                           customFields['Ano/Série Escolar'] ||
                ...
                difficultyLevel: consolidatedCustomFields['Tipo de Aula'] ||
                                ...
                                consolidatedCustomFields['tipoAula'] ||
                                customFields['Tipo de Aula'] ||
                ...
                materials: consolidatedCustomFields['Materiais/Recursos'] ||
                          ...
                          customFields['Materiais/Recursos'] ||
                ...
                evaluation: consolidatedCustomFields['Observações do Professor'] ||
                           ...
                           customFields['Observações do Professor'] ||
              ...
              enrichedFormData = {
                ...
                disciplina: consolidatedCustomFields['Disciplina'] || autoFormData.disciplina || customFields['Disciplina'] || activity?.customFields?.disciplina || '',
              ...
              enrichedFormData = {
                ...
                theme: consolidatedCustomFields['Tema'] || autoFormData.theme || activity.theme || customFields['Tema'] || '',
                subject: consolidatedCustomFields['Disciplina'] || autoFormData.subject || 'Matemática' || customFields['Disciplina'] || '',
                schoolYear: consolidatedCustomFields['Ano de Escolaridade'] || autoFormData.schoolYear || '6º Ano - Ensino Fundamental' || customFields['Ano de Escolaridade'] || '',
                ...
                context: consolidatedCustomFields['Contexto de Aplicação'] || autoFormData.context || customFields['Contexto de Aplicação'] || '',
                format: consolidatedCustomFields['Formato do Quiz'] || autoFormData.format || customFields['Formato do Quiz'] || '', // New field
                timePerQuestion: consolidatedCustomFields['Tempo por Questão'] || autoFormData.timePerQuestion || customFields['Tempo por Questão'] || '', // New field
                quadroInterativoCampoEspecifico: consolidatedCustomFields['quadroInterativoCampoEspecifico'] || autoFormData.quadroInterativoCampoEspecifico || customFields['quadroInterativoCampoEspecifico'] || '',
              };
              ...
              enrichedFormData = {
                ...
                subject: customFields['Disciplina'] || customFields['disciplina'] || autoFormData.subject || 'Geral',
                schoolYear: customFields['Ano de Escolaridade'] || customFields['anoEscolaridade'] || autoFormData.schoolYear || 'Ensino Médio',
                difficultyLevel: customFields['Nível de Dificuldade'] || customFields['nivelDificuldade'] || autoFormData.difficultyLevel || 'Médio',
                objectives: customFields['Objetivos'] || autoFormData.objectives || `Facilitar o aprendizado sobre ${customFields['Tema'] || customFields['tema'] || 'o tema'}`,
                instructions: customFields['Instruções'] || autoFormData.instructions || 'Use os flash cards para estudar e revisar o conteúdo',
                evaluation: customFields['Critérios de Avaliação'] || autoFormData.evaluation || 'Avalie o conhecimento através da prática com os cards',
              };
...
```// Analysis: The code now includes a robust system for automatically synchronizing activity data with the Neon database after saving, ensuring data integrity and persistence.
<replit_final_file>
import React, { useState, useEffect, useCallback } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { X, Eye, Settings, FileText, Play, Download, Edit3, Copy, Save, BookOpen, GamepadIcon, PenTool, Calculator, Beaker, GraduationCap, Sparkles } from 'lucide-react';
