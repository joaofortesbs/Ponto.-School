...
function generateAvaliacaoDiagnostica(ctx: ContextInfo): string {
  ...
  for (let i = 1; i <= Math.min(ctx.quantidade, 10); i++) {
    questoes.push({
      ...
      alternativas: [
        { letra: 'A', texto: `Alternativa A sobre ${ctx.tema}` },
        { letra: 'B', texto: `Alternativa B sobre ${ctx.disciplina}` },
        { letra: 'C', texto: `Alternativa C com aplicação prática` },
        { letra: 'D', texto: `Alternativa D complementar` },
      ],
      respostaCorreta: 'A',
