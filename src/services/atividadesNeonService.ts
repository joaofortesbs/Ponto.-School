...
class AtividadesNeonService {
  ...
  async salvarAtividade(id: string, userId: string, tipo: string, dadosAtividade: any, stars?: number): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      console.log('💾 Salvando atividade no Neon:', { id, userId, tipo, stars });
      ...
      const response = await fetch(API_BASE_URL, {
        ...
        body: JSON.stringify({
          ...
          stars: stars !== undefined ? stars : 100
        })
      });
