...
// Interfaces para manipulação de conversas
...
// Função para salvar o histórico da conversa no localStorage e no Supabase
async function saveConversationHistory(sessionId: string, history: ChatMessage[]): Promise<void> {
  try {
    ...
    // Salvar no Supabase para persistência e acesso entre dispositivos
    await saveConversationToSupabase(sessionId, history);
  } catch (error) {
...
async function saveConversationToSupabase(sessionId: string, history: ChatMessage[]): Promise<void> {
  try {
    // Verificar se há uma sessão autenticada
    const { data: { session } } = await supabase.auth.getSession();
    if (!session || !session.user) {
      console.log('Usuário não autenticado, não será possível salvar a conversa');
      return;
    }

    // Ignorar mensagens do sistema (primeira mensagem) ao criar a preview
    const userMessages = history.filter(msg => msg.role === 'user');
    const assistantMessages = history.filter(msg => msg.role === 'assistant');
    
    if (userMessages.length === 0 || assistantMessages.length === 0) {
      // Não salvar se não houver pelo menos uma mensagem do usuário e uma resposta
      return;
    }

    // Criar mensagens no formato necessário para o banco
    const mensagens: Mensagem[] = history.slice(1).map(msg => ({
      id: uuidv4(),
    ...
    // Título baseado na primeira mensagem do usuário (limitado a 50 caracteres)
    const firstUserMessage = userMessages[0].content;
    const titulo = firstUserMessage.length <= 50 
                  ? firstUserMessage 
                  : `${firstUserMessage.substring(0, 47)}...`;

    // Preview baseado na última resposta do assistente
    const lastAssistantMessage = assistantMessages[assistantMessages.length - 1].content;
    const preview = lastAssistantMessage.length <= 100 
                  ? lastAssistantMessage 
                  : `${lastAssistantMessage.substring(0, 97)}...`;

    // Extrair tags com base no conteúdo (palavras-chave)
    const tags = extractKeywords(userMessages[0].content, 3);

    // Determinar o tipo de conversa
    const tipo = determineConversationType(userMessages[0].content);

    // Gerar análise automática
    const analise = generateAnalysis(history);
...
function extractKeywords(text: string, maxKeywords: number = 3): string[] {
  ...
  const stopwords = ['a', 'ao', 'aos', 'aquela', 'aquelas', 'aquele', 'aqueles', 'aquilo', 'as', 'até', 'com', 'como', 'da', 'das', 'de', 'dela', 'delas', 'dele', 'deles', 'depois', 'do', 'dos', 'e', 'ela', 'elas', 'ele', 'eles', 'em', 'entre', 'era', 'eram', 'éramos', 'essa', 'essas', 'esse', 'esses', 'esta', 'estas', 'este', 'estes', 'eu', 'foi', 'fomos', 'for', 'foram', 'forem', 'formos', 'fosse', 'fossem', 'fôssemos', 'há', 'isso', 'isto', 'já', 'lhe', 'lhes', 'mais', 'mas', 'me', 'mesmo', 'meu', 'meus', 'minha', 'minhas', 'muito', 'na', 'não', 'nas', 'nem', 'no', 'nos', 'nós', 'nossa', 'nossas', 'nosso', 'nossos', 'num', 'numa', 'o', 'os', 'ou', 'para', 'pela', 'pelas', 'pelo', 'pelos', 'por', 'qual', 'quando', 'que', 'quem', 'sáo', 'se', 'sem', 'seja', 'sejam', 'sejamos', 'ser', 'será', 'serão', 'seria', 'seriam', 'seríamos', 'seu', 'seus', 'só', 'somos', 'sou', 'sua', 'suas', 'também', 'te', 'tem', 'tém', 'temos', 'tenha', 'tenham', 'tenhamos', 'tenho', 'terá', 'terão', 'terei', 'teremos', 'teria', 'teriam', 'teríamos', 'teu', 'teus', 'tu', 'tua', 'tuas', 'um', 'uma', 'você', 'vocês', 'vos'];
  ...
  // Ordenar por frequência e pegar as top keywords
...
// Função para determinar o tipo de conversa
function determineConversationType(text: string): "explicacao" | "quiz" | "correcao" | "resumo" | "fluxograma" {
  const lowerText = text.toLowerCase();
  
  if (lowerText.includes('quiz') || lowerText.includes('teste') || lowerText.includes('simulado') || lowerText.includes('questões') || lowerText.includes('perguntas')) {
    return 'quiz';
  } else if (lowerText.includes('corrigir') || lowerText.includes('correção') || lowerText.includes('avaliar') || lowerText.includes('revisão') || lowerText.includes('feedback')) {
    return 'correcao';
  } else if (lowerText.includes('resumo') || lowerText.includes('sintetizar') || lowerText.includes('resumir')) {
    return 'resumo';
  } else if (lowerText.includes('fluxograma') || lowerText.includes('diagrama') || lowerText.includes('mapa mental') || lowerText.includes('esquema')) {
    return 'fluxograma';
  } else {
    return 'explicacao';
  }
...
// Função para gerar análise automática da conversa
function generateAnalysis(history: ChatMessage[]) {
  // Remover a mensagem do sistema
  const messageHistory = history.slice(1);
  
  // Contar número de trocas (pergunta/resposta)
  const exchanges = Math.floor(messageHistory.length / 2);
  
  // Calcular qualidade com base em alguns heurísticos simples
  // Neste exemplo, usamos o comprimento da resposta e número de trocas como proxies
  const avgResponseLength = messageHistory
    .filter(msg => msg.role === 'assistant')
    .reduce((sum, msg) => sum + msg.content.length, 0) / (messageHistory.length / 2);
  
  // Qualidade simples baseada no tamanho médio das respostas e número de interações
  let qualidade = Math.min(100, 60 + (avgResponseLength / 1000 * 20) + (exchanges * 5));
  
  // Extrair palavras-chave de todas as mensagens
  const allContent = messageHistory.map(msg => msg.content).join(' ');
  const palavrasChave = extractKeywords(allContent, 5);
  
  // Gerar sugestões baseadas no tipo de conteúdo
  const sugestoes = [
    "Revisar este conteúdo em 7 dias",
    "Criar flashcards com os conceitos principais"
  ];
  
  if (exchanges >= 3) {
    sugestoes.push("Aprofundar o estudo deste tópico");
  }
...
// Função para carregar conversas do Supabase
export async function loadUserConversations(): Promise<Conversa[]> {
  try {
    // Verificar se há uma sessão autenticada
    const { data: { session } } = await supabase.auth.getSession();
    if (!session || !session.user) {
      console.log('Usuário não autenticado, não será possível carregar conversas');
      return [];
    }
    ...
    const conversas: Conversa[] = data.map(item => ({
      id: item.id,
      titulo: item.titulo,
      tipo: item.tipo,
      data: new Date(item.updated_at),
      tags: item.tags || [],
...
export async function updateConversation(id: string, updates: Partial<Conversa>): Promise<boolean> {
  try {
    const { data, error } = await supabase
      .from('user_conversations')
      .update(updates)
