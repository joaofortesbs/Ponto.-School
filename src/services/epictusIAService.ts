...
export const getResponse = async (message: string): Promise<string> => {
    try {
      ...
      const processingTime = hasAttachedFiles ? 2500 : 1500;
      await new Promise(resolve => setTimeout(resolve, processingTime));

      // Resposta simulada da IA com base no conteúdo da mensagem
      let response = "";

      if (hasAttachedFiles) {
        // Extrai informações sobre os arquivos da mensagem
        const fileSection = message.split("Arquivos anexados:")[1]?.split("Obs:")[0].trim();
        if (fileSection) {
          const fileLines = fileSection.split('\n').filter(line => line.trim().startsWith('-'));

          response = `Recebi sua mensagem junto com ${fileLines.length} arquivo(s) anexado(s). `;

          // Verifica tipos de arquivos para personalizar a resposta
          const hasImages = fileLines.some(line => line.includes("image/") || line.includes(".jpg") || line.includes(".png"));
          const hasPdfs = fileLines.some(line => line.includes("application/pdf") || line.includes(".pdf"));
          const hasDocuments = fileLines.some(line => 
            line.includes("application/msword") || 
            line.includes("application/vnd.openxmlformats") ||
            line.includes(".doc") || 
            line.includes(".docx")
          );

          if (hasImages) {
            response += "Identifiquei algumas imagens que podem ser relevantes para nossa conversa. ";
          }

          if (hasPdfs) {
            response += "Notei que você anexou documentos PDF. ";
          }

          if (hasDocuments) {
            response += "Vejo que você compartilhou documentos do Word. ";
          }

          // Adiciona uma resposta contextual ao texto da mensagem
          const userText = message.split("Arquivos anexados:")[0].trim();
          if (userText) {
            response += `\n\nSobre sua mensagem: ${userText}\n\n`;

            if (userText.toLowerCase().includes("olá") || userText.toLowerCase().includes("oi")) {
              response += "Olá! Como posso ajudar você hoje com os arquivos que você enviou?";
            } else if (userText.toLowerCase().includes("ajuda")) {
              response += "Estou aqui para ajudar! Vou analisar os arquivos que você enviou.";
            } else if (userText.toLowerCase().includes("matemática") || userText.toLowerCase().includes("cálculo")) {
              response += "Posso ajudar com problemas matemáticos nos arquivos que você compartilhou. Qual é a sua dúvida específica?";
            } else {
              response += "Analisei o conteúdo que você enviou. Posso ajudar com mais informações ou esclarecimentos sobre isso?";
            }
          } else {
            response += "Como posso ajudar com esses arquivos? Gostaria de uma análise específica de algum deles?";
          }
        }
        ...
        response = "Estou aqui para ajudar! O que você gostaria de saber?";
      } else if (message.toLowerCase().includes("matemática") || message.toLowerCase().includes("cálculo")) {
        response = "Posso ajudar com problemas matemáticos. Qual é a sua dúvida específica?";
      } else if (message.toLowerCase().includes("história")) {
        response = "História é fascinante! Qual período ou evento histórico você gostaria de explorar?";
      } else if (message.toLowerCase().includes("ciência")) {
        response = "A ciência nos ajuda a entender o mundo! Qual área científica você está interessado?";
      } else if (message.toLowerCase().includes("arquivo")) {
        response = "Vi que você mencionou arquivos. Você pode anexá-los à conversa para que eu possa ajudar a analisar ou discutir o conteúdo deles.";
      } else {
        response = "Entendi seu questionamento. Vamos explorar esse assunto juntos. Você poderia fornecer mais detalhes para que eu possa ajudar melhor?";
      }

      return response;
    } catch (error) {
      console.error("Erro ao obter resposta da IA:", error);
      return "Desculpe, tive um problema ao processar sua solicitação. Por favor, tente novamente em alguns instantes.";
    }
  };