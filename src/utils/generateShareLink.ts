...
function generateUniqueCode(): string {
  // Gera um código de 8 caracteres único usando nanoid
  return nanoid(8);
}
...
function saveSharedActivity(activityId: string, activityData: any): string {
  ...
  const sharedActivityData: SharedActivityData = {
    id: activityId,
    uniqueCode,
    title: activityData.title || activityData.personalizedTitle || `Atividade ${activityId}`,
    description: activityData.description || activityData.personalizedDescription || 'Atividade educacional compartilhada',
    content: activityData.content || activityData,
    createdAt: new Date().toISOString(),
    activityType: activityData.type || activityData.categoryId || activityId,
    isPublic: true
  };
  ...
  // Salvar mapeamento activityId -> uniqueCode para reutilização
...
function getOrCreateUniqueCode(activityId: string): string {
  ...
  if (existingCode && validarCodigoUnico(existingCode)) {
...
function getActivityData(activityId: string): any {
  console.log(`🔍 Buscando dados para atividade: ${activityId}`);
  ...
  const possibleKeys = [
    ...
    activityId // Tentar o ID direto também
  ];
  ...
  for (const key of possibleKeys) {
    ...
    if (data) {
      try {
        ...
        if (parsedData && (parsedData.title || parsedData.content || parsedData.questoes || parsedData.sequenciaDidatica)) {
          console.log(`📦 Dados da atividade encontrados em: ${key}`, parsedData);
          return parsedData;
        }
      } catch (error) {
  ...
  console.log(`⚠️ Dados específicos não encontrados para ${activityId}, criando dados básicos`);
  return {
    id: activityId,
    title: `Atividade: ${activityId.charAt(0).toUpperCase() + activityId.slice(1).replace(/-/g, ' ')}`,
    description: 'Atividade educacional compartilhada publicamente',
    content: 'Esta é uma atividade educacional disponível publicamente. O conteúdo específico será carregado dinamicamente.',
    type: activityId,
    createdAt: new Date().toISOString()
  };
}
...
export function gerarLinkCompartilhamento(activityId: string): string {
  ...
  try {
    ...
    // Gerar link com código único como parâmetro na URL
...
export function getSharedActivityByCode(uniqueCode: string): SharedActivityData | null {
  try {
    console.log(`🔍 Buscando atividade pelo código: ${uniqueCode}`);
    
    const data = localStorage.getItem(`shared_activity_${uniqueCode}`);
    if (data) {
      const parsedData = JSON.parse(data);
      console.log(`✅ Atividade encontrada pelo código:`, parsedData);
      return parsedData;
    }
    
    console.log(`❌ Nenhuma atividade encontrada para o código: ${uniqueCode}`);
    return null;
  } catch (error) {
