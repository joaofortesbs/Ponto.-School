...
import { Plus, FileText, Clock, MoreVertical, Eye, Edit2, Trash2, Share2 } from 'lucide-react';
...
const AtividadesGrid: React.FC<AtividadesGridProps> = ({ searchTerm }) => {
  ...
  const [filteredAtividades, setFilteredAtividades] = useState<Atividade[]>(mockAtividades);
  ...
  const AtividadeCard = ({ atividade, index }: { atividade: Atividade; index: number }) => (
    ...
    >
      <div className="absolute top-3 right-3 z-10">
        <button
          onClick={() => setActiveMenu(activeMenu === atividade.id ? null : atividade.id)}
      ...
      <div className="p-4">
        ...
        <p className="text-white/50 text-xs mb-2">{atividade.tipo}</p>
        <div className="flex items-center gap-1 text-white/40 text-xs">
          <Clock className="w-3 h-3" />
          <span>{atividade.criadoEm}</span>
        </div>
      </div>
    </motion.div>
  );
