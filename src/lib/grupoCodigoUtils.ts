...
export const verificarSeCodigoExisteDetalhado = async (
  codigo: string
): Promise<{
  existe: boolean;
  detalhes?: any;
  ...
  if (!codigo) {
    return { existe: false, error: "Código não fornecido" };
  }
  ...
  try {
    ...
    // Verificar na tabela de códigos
    const { data, error } = await supabase
      .from("codigos_grupos_estudo")
      .select("*")
    ...
    if (error) {
      console.error("Erro ao verificar código na tabela:", error);

      // Fallback: verificar na tabela de grupos
      try {
        const { data: grupoData, error: grupoError } = await supabase
          .from("grupos_estudo")
          .select("*")
        ...
        if (grupoError) {
          ...
          return { existe: false, error: grupoError };
        }

        if (grupoData) {
          return { existe: true, detalhes: grupoData };
        }
      } catch (fallbackError) {
    ...
    if (data) {
      return { existe: true, detalhes: data };
    }
...
// Função para verificar relação de um usuário com um grupo
export const verificarRelacaoUsuarioComGrupo = async (userId: string, grupoId: string): Promise<{ 
  isMembro: boolean, 
  isCriador: boolean,
  error?: any 
}> => {
  if (!userId || !grupoId) {
    return { isMembro: false, isCriador: false, error: "ID de usuário ou grupo não fornecido" };
  }

  try {
    const { data, error } = await supabase
      .from("grupos_estudo")
      .select("user_id, membros_ids")
      .eq("id", grupoId)
      .single();

    if (error) {
      console.error("Erro ao verificar relação usuário-grupo:", error);
      return { isMembro: false, isCriador: false, error };
    }

    const isCriador = data.user_id === userId;
    const membrosIds = data.membros_ids || [];
    const isMembro = membrosIds.includes(userId);

    return { isMembro, isCriador };
  } catch (error) {
    console.error("Exceção ao verificar relação usuário-grupo:", error);
    return { isMembro: false, isCriador: false, error };
  }
};

// Função para buscar um grupo pelo código
export const buscarGrupoComCodigo = async (codigo: string) => {
  ...
  try {
    ...
    // Primeiro buscar na tabela de códigos
    const { data, error } = await supabase
      .from("codigos_grupos_estudo")
      .select("*")
      .eq("codigo", codigoFormatado)
    ...
    if (error) {
      ...
      // Fallback: buscar diretamente na tabela de grupos
      const { data: grupoData, error: grupoError } = await supabase
        .from("grupos_estudo")
        .select("*")
        .eq("codigo", codigoFormatado)
      ...
      if (grupoError) {
        console.error("Erro ao buscar por código na tabela de grupos:", grupoError);
    ...
    if (data) {
      // Se encontrou na tabela de códigos, verificar se o grupo existe
      try {
        const { data: grupoData, error: grupoError } = await supabase
          .from("grupos_estudo")
          .select("*")
        ...
        // Se não encontrou o grupo, retornar os dados da tabela de códigos
        return { success: true, data };
      } catch (grupoError) {
        console.error("Erro ao buscar grupo relacionado ao código:", grupoError);
        // Retornar os dados da tabela de códigos como fallback
        return { success: true, data };
      }
    }
    ...
    console.error("Exceção ao buscar grupo por código:", error);
    return { success: false, error };
  }
};

// Wrapper para verificar se o código existe
export const verificarSeCodigoExiste = async (codigo: string): Promise<boolean> => {
  try {
    const { existe } = await verificarSeCodigoExisteDetalhado(codigo);
    return existe;
  } catch (error) {
