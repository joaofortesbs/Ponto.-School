...
app.post('/api/codigos-grupo/validar-acesso', (req, res) => {
  try {
    ...
    return res.status(500).json({
      sucesso: false,
      mensagem: 'Erro interno do servidor ao validar acesso'
    });
  }
});

// Rota para enviar solicitação de acesso a grupo privado
app.post('/api/codigos-grupo/solicitar-acesso', async (req, res) => {
  try {
    const { codigo, userId, mensagem } = req.body;
    
    console.log('API: Solicitando acesso - código:', codigo, 'userId:', userId);
    ...
    const { data: solicitacoesExistentes, error: erroConsulta } = await supabase
      .from('grupos_solicitacoes')
      .select('id, status')
      .eq('grupo_id', codigoInfo.grupoId)
      .eq('user_id', userId)
      .eq('status', 'pendente');
      
    if (erroConsulta) {
      console.error('Erro ao verificar solicitações existentes:', erroConsulta);
    } else if (solicitacoesExistentes && solicitacoesExistentes.length > 0) {
      return res.json({
        sucesso: false,
        codigo: 'solicitacao_existente',
        mensagem: 'Você já tem uma solicitação pendente para este grupo'
      });
    }
    ...
    // Atualizar a cache local
    const solicitacoesCache = db.solicitacoes || [];
    ...
    db.solicitacoes = solicitacoesCache;
    saveDb(db);
    
    console.log('API: Solicitação registrada com sucesso');
    return res.json({
      sucesso: true,
      mensagem: 'Solicitação enviada com sucesso',
      grupoNome: codigoInfo.nome
    });
  } catch (error) {
    console.error('Erro ao processar solicitação de acesso:', error);
    return res.status(500).json({
      sucesso: false,
      mensagem: 'Erro interno do servidor ao processar solicitação'
    });
  }
});d: codigoInfo.grupoId,
        nome: codigoInfo.nome
      });
    }
