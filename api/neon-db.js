...
class NeonDBManager {
  ...
  async createActivity(activityData) {
    ...
    const query = `
      ...
      VALUES ($1, $2, $3, $4, $5)
      RETURNING id, user_id, activity_code, type, title, content, created_at, updated_at
    `;
  ...
  async findActivityByCode(activity_code) {
    try {
      console.log('🔍 Buscando atividade por código:', activity_code);
      const query = 'SELECT * FROM activities WHERE activity_code = $1';
      const result = await this.executeQuery(query, [activity_code]);
  ...
  async findActivitiesByUser(user_id) {
    try {
      console.log('🔍 Buscando atividades por usuário:', user_id);
      const query = 'SELECT * FROM activities WHERE user_id = $1 ORDER BY created_at DESC';
      const result = await this.executeQuery(query, [user_id]);

      console.log('📊 Atividades encontradas:', result.data.length);
      ...
      console.error('❌ Erro ao buscar atividades por usuário:', error);
      return {
        success: false,
        error: error.message,
        data: []
  ...
  // Atualizar atividade
  async updateActivity(activity_code, updateData) {
  ...
  // Deletar atividade
  async deleteActivity(activity_code) {
    const query = 'DELETE FROM activities WHERE activity_code = $1 RETURNING id, activity_code';
    return await this.executeQuery(query, [activity_code]);
  }
}
